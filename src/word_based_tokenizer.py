...
class WordBasedTokenizer(object):
    ...
    def _find_all_words(self, sentence, sentence_length):
        ...
        for i in range(1, sentence_length - 1):
            ...
            graph.insert_start_word(Node(word, self._calculate_weight(freq),
                                         freq=freq))  # 将单字加入起始节点
            ...
            while j < sentence_length - 1:
                source = self.word_dict.is_in(
                    fragment)  # 判断fragment是否在词库（是词 或 词的前缀）
                ...
                if freq:  # 如果是词（而不是前缀），添加此节点到词图
                    ...
                    graph.insert_start_word(
                        Node(fragment, self._calculate_weight(freq), source,
                             freq))
    ...
    def _find_end_words_list(start_char_index, all_words):
        ...
        for end_index in all_words[start_char_index]:  # 对当前位置的所有节点的end_index
            base_index = sum([len(all_words[i]) for i in
                              range(end_index + 1)])  # 当前节点的node序号
            end_words = [base_index + count for count in
                         range(len(all_words[end_index + 1]))]  # 终止节点的node序号
            yield end_words
    ...
    def seg(self, sentence):
        ...
        """
        sentence = [WordGraph.NODE_S.key] + list(sentence) + [
            WordGraph.NODE_E.key]
