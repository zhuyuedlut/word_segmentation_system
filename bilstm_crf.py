...
class BiLSTM_CRF(nn.Module):
    ...
    def _viterbi_decode(self, feats):
        ...
        for feat in feats:
            ...
            for next_tag in range(self.tagset_size):
                ...
                # 这里加上self.transitions[next_tag]就是转移到next_tag的得分
                # （这里暂时没有将发射score添加进来，因为最大值并不依赖它们。我们下面会加它。）
                next_tag_var = forward_var + self.transitions[next_tag]
                # 记录在t时刻转移到next_tag的状态的前一状态
                # 因为维特比算法只有算到了最后的状态才知道哪个是最好的状态，然后在反向推导路径所以需要保留一下每个状态之前的最佳状态
                best_tag_id = argmax(next_tag_var)
                bptrs_t.append(best_tag_id)
                viterbivars_t.append(next_tag_var[0][best_tag_id].view(1))

            # 现在将所有发射score相加，更新forward_var，为了计算t+1时刻的状态
    ...
    def neg_log_likelihood(self, sentence, tags):
        """
        已知真实标签，计算crf loss
        reference: https://tmylla.github.io/2020/10/12/BiLSTM+CRF%E8%AF%A6%E8%A7%A3/
